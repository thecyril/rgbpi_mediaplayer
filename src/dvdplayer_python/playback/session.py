...
class PlaybackSession:
    ...
    def _send(self, payload: dict) -> dict:
        ...
        ``osd-overlay`` lifetimes to the client (= socket connection) —
        every disconnect tears the overlays down — so we must hold one
        socket open for the whole session, not one per request.
        ...
        ``--input-ipc-server`` and gets a new libmpv client.
        """
        if self.backend != "mpv":
